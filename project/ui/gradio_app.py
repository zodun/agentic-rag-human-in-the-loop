...
def create_gradio_ui():
    ...
    def approve_reply_handler(thread_id, final_text):
        ...
        where = "Sent to Slack. " if "Slack" in r["delivered_to"] else ""
        return f"{where}A copy is saved at `{r['path']}`."
        ...
        if config.HITL_REPLY_ENABLED:
            with gr.Tab("Draft Reply"):
                ...
                with gr.Row():
                    ...
                    revise_btn = gr.Button("Rewrite", scale=1)
                with gr.Row():
                    approve_btn = gr.Button("Approve & send", variant="primary")
                    discard_btn = gr.Button("Throw away", variant="stop")
